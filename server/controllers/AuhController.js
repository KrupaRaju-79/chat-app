...
export const signup = async (req, res) => {
  try {
    console.log("Signup request received: ❤️❤️❤️❤️");
    console.log("Signup request received: ❤️❤️❤️❤️", req.body);
...
export const getUserInfo = async (request, response, next) => {
  try {
    ...
    if (!userData) {
      return response
        .status(404)
        .json({ message: "User by the given id not found" });
    }
    ...
    response
      .status(500)
      .json({ message: "Internal server error while fetching user info." });
  }
};
...
export const setupProfile = async (request, response, next) => {
  try {
    ...
    if (!name) {
      return response
        .status(400)
        .json({ message: "Username is required for profile setup." });
    }
    ...
    if (!userData) {
      return response
        .status(404)
        .json({ message: "User not found for profile setup." });
    }
    ...
    response
      .status(500)
      .json({ message: "Internal server error during profile setup." });
  }
};
...
export const updateProfile = async (request, response, next) => {
  try {
    ...
    if (Object.keys(fieldsToUpdate).length === 0) {
      return response
        .status(400)
        .json({ message: "No fields provided for profile update." });
    }
    ...
    if (!userData) {
      return response
        .status(404)
        .json({ message: "User not found for profile update." });
    }
...
export const uploadImage = async (req, res) => {
  try {
    ...
    if (!req.file) {
      return res
        .status(400)
        .json({ message: "No image file provided or file type not allowed." });
    }
    ...
    if (currentUser && currentUser.image) {
      ...
      try {
        ...
        console.warn(
          `Failed to delete old image file ${oldImagePath}: ${fileError.message}`
        );
      }
    }
    ...
    if (error.code === "LIMIT_FILE_SIZE") {
      return res
        .status(413)
        .json({ message: `Image size limit exceeded. Max 2MB allowed.` });
    }
    if (
  ...
  try {
    ...
    if (oldImageUrl) {
      ...
      try {
        ...
        console.warn(
          `Could not delete physical image file ${oldImagePath}: ${fileError.message}`
        );
      }
    }
