...
export const SocketProvider = ({ children }) => {
  ...
  const { userInfo, addMessage, replaceTemporaryMessage } = useAppStore();
  ...
  useEffect(() => {
    if (userInfo && userInfo.id) {
      ...
      newSocket.on('connect', () => {
        console.log(
          `[SocketContext] Socket connected with ID: ${newSocket.id}`
        );
      });
      ...
      console.log(
        `[SocketContext] Attempting to connect socket for user: ${userInfo.id}`
      );
    } else if (!userInfo?.id && socket.current) {
      console.log(
        '[SocketContext] User info not available, disconnecting socket.'
      );
      socket.current.disconnect();
      socket.current = null;
      setCallState(null);
    ...
    return () => {
      if (socket.current) {
        console.log(
          '[SocketContext] Cleanup: Disconnecting socket and clearing ref.'
        );
        socket.current.disconnect();
        socket.current = null;
        setCallState(null);
    ...
    if (socket.current) {
      const handleReceiveMessage = (message) => {
        ...
        const normalizedMessage = {
          ...
          recipient:
            typeof message.recipient === 'object' && message.recipient !== null
              ? message.recipient
              : {
        ...
        console.log(
          '[SocketContext DEBUG] Normalized message:',
          normalizedMessage
        );
        ...
        const isRelevantToSelectedChat =
          ...
          ((normalizedMessage.sender?._id === currentSelectedChatData._id &&
            ...
            (normalizedMessage.sender?._id === currentUserInfo.id &&
              normalizedMessage.recipient?._id ===
                currentSelectedChatData._id));

        if (isRelevantToSelectedChat) {
          console.log(
            '[SocketContext DEBUG] Message is relevant to selected chat. Calling replaceTemporaryMessage.'
          );
          replaceTemporaryMessage(normalizedMessage);
        } else {
          console.log(
            '[SocketContext DEBUG] Message not relevant to selected chat. Skipping UI update for now.'
          );
        }
      };
      ...
      const handleRecieveChannelMessage = (message) => {
        console.log(
          '[SocketContext DEBUG] Raw channel message received:',
          message
        );
        ...
        console.log(
          '[SocketContext DEBUG] Normalized channel message:',
          normalizedMessage
        );
        ...
        const isMyOwnChannelMessage =
          normalizedMessage.sender?._id === currentUserInfo.id;

        if (isRelevantToSelectedChannel) {
          if (isMyOwnChannelMessage) {
            console.log(
              '[SocketContext DEBUG] My own channel message received. Calling replaceTemporaryMessage.'
            );
            replaceTemporaryMessage(normalizedMessage);
          } else {
            console.log(
              '[SocketContext DEBUG] Channel message from another user received. Calling addMessage.'
            );
            addMessage(normalizedMessage);
          }
        } else {
          console.log(
            '[SocketContext DEBUG] Channel message not relevant to selected channel. Skipping UI update for now.'
          );
        }
      };
      ...
      const handleIncomingCall = ({ from, callType }) => {
        if (callState) {
          ...
          toast.warning(
            `Missed ${callType} call from ${from} - already in a call.`
          );
          return;
        }
